...
class SettingViewController: UIViewController {
    ...
    @IBOutlet weak var disuplayNameTextField: UITextField!
    
    //表示名変更ボタンをタップした時に呼ばれるメソッド
    @IBAction func handleChangeButton(_ sender: Any) {
        if let displayName = disuplayNameTextField.text {
    ...
    @IBAction func handleLogoutButton(_ sender: Any) {
        ...
        //ログイン画面から戻ってきた時のためにホーム画面（index = 0）を設定している状態にしておく
    ...
    override func viewDidLoad() {
        ...
        let user = Auth.auth().currentUser
        if let user = user {
            disuplayNameTextField.text = user.displayName
